...
int main() {
    ...
    try {
        Map *map = MapLoader::parse("../Resource/test_initial_territory_index_error.map");
        delete map;
    } catch (const std::exception &e) {
        cout << e.what() << endl;
    }
    ...
    try {
        Map *map = MapLoader::parse("../Resource/test_bordering_territory_index_error.map");
        delete map;
    } catch (const std::exception &e) {
        cout << e.what() << endl;
    }
    ...
    try {
        Map *map = MapLoader::parse("../Resource/test_missing data.map");
        delete map;
    } catch (const std::exception &e) {
        cout << e.what() << endl;
    }
    ...
    try {
        Map *map = MapLoader::parse("../Resource/test_missing_border_info.map");
        delete map;
    } catch (const std::exception &e) {
        cout << e.what() << endl;
    }
